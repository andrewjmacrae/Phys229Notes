...
Instead of creating a junction between a P-type and N-type semiconductor, a Schottky diode is formed with a doped semiconductor (P or N) and a metal. This design leads to a very fast response time and a typically lower voltage drop ($0.1$~V to $0.2$~V.)

\section{Sneak Peek Ahead: Bipolar Junction Transistors}
While diodes form an incredibly useful circuit element by combining a PN junction, an even more useful device can be made by adding one more layer, forming a NPN or PNP junction (see figure ([{XXX}])). By injecting current into the middle layer, we will be able to control the current through the device as a whole, effectively having a current-controlled resistor. For this reason this device was called the ``current TRANSfer resISTOR'' but is now just called a \textit{transistor}. The NPN or PNP can be seen as a pair of diodes placed in opposite directions, i.e. having opposite polarity. Thus this is technically called a ``bipolar junction transistor'' (BJT) but is typically just referred to as a transistor. There is another common type of transistor called the ``field effect transistor'' (FET) which is commonly just called a ``FET'' in the lab.\footnote{Among FETs there is the MOSFET and JFET, and so on. The topic of transistors is a fairly lengthy one and we will concentrate soleley on NJTs in this course.}

\subsection{Physical Operation of Transistors}
Tizzle Dizzle
